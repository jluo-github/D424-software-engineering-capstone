...
public class AddProductController {
  ...
  @PostMapping("/{productID}/associatepart/{partID}")
  public ResponseEntity<?> associatePart(@PathVariable("partID") int partId, @PathVariable("productID") int productId) {
  ...
  @PostMapping("/{productID}/removepart/{partID}")
  public ResponseEntity<?> removePart(@PathVariable("partID") int theID, @PathVariable("productID") int productId) {
    // Check if the product has a name
    Product product1 = productService.findById(productId);
    if (product1.getName() == null) {
      return ResponseEntity.badRequest().body("Product is not valid");
